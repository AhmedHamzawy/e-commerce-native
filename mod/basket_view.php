...
if (!empty($session)) {
	...
	foreach($session as $key => $value) {
		$out[$key] = get_object_vars($objCatalogue->displayProduct($key));
	}
}
?>
...
<?php if (!empty($out)) { ?>
	...
	<table cellpadding="0" cellspacing="0" border="0"
     class="mdl-data-table mdl-js-data-table mdl-data-table mdl-shadow--2dp bb">
		<tr>
		...
		<tr>
			...
			<td class="ta_r">&pound;<?php echo number_format($objBasket->itemTotal($item['price'], 
