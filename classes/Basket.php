...
class Basket {
	...
	public function subtotal() {
		...
		if (!$this->_empty_basket) {
			foreach($_SESSION['basket'] as $key => $basket) {
				 $product = $this->_inst_catalogue->displayProduct($key);
	...
	public static function activeButton($sess_id) {
		...
		if(isset($_SESSION['basket'][$sess_id])) {
			$id = 0;
			$label = "<i class=\"material-icons\">remove_shopping_cart</i>";
		} else {
			$id = 1;
			$label = "<i class=\"material-icons\">add_shopping_cart</i>";
		}
		
		$out  = "<a href=\"#\" class=\"add_to_basket mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect 
		mdl-button--colored";
		$out .= $id == 0 ? " red" : null;
		$out .= "\" rel=\"";
		$out .= $sess_id."_".$id;
