...
if (!empty($id)) {
	...
	if (!empty($category)) {
		...
		if ($objForm->isPost('name')) {
			...
			if ($objCategories->duplicateCategory($name, $id)) {
				$objValid->add2Errors('name_duplicate');
			}
			
			if ($objValid->isValid()) {
				
				if ($objCategories->updateCategory($objValid->_post, $id)) {
					
					$category = $objValid->_post["name"];
					Helper::log_action('Category Updated' , "A Category {$category} is updated");
...
?>
    ...
    <header class="mdl-layout__header">
        ...
        <span class="mdl-layout-title mdl-typography--text-center mdl-typography--display-3">Categories :: Edit</span>
    ...
    <h2 class="mdl-card__title-text">Categories :: Edit</h2>
    </div>
		...
		<div class="mdl-textfield mdl-js-textfield">
				<label for="name" class="mdl-textfield__label">Name: *</label>
					...
					<input type="text" name="name" id="name" 
						value="<?php echo $objForm->stickyText('name', $category->name); ?>" class="mdl-textfield__input" />
		</div>		
                ...
                <select name="position" id="position" class="mdl-textfield__input">
					...
					<?php for($count = 1 ; $count <= ($categories_count + 1) ; $count++){ ?>
                         ...
                         <?php echo $objForm->stickySelect('position', $count , $category->position) ?>>
                         <?php echo $count ?>
                         </option>
